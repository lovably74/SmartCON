...
export default function DashboardLayout({ children, role }: DashboardLayoutProps) {
  ...
  const MobileNav = () => (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-background border-t z-50 flex justify-around items-center h-16 px-2 pb-safe">
      <Link href={`/${role}/dashboard`}>
        <a className={cn(
          "flex flex-col items-center justify-center w-full h-full space-y-1",
          location === `/${role}/dashboard` ? "text-primary" : "text-muted-foreground"
        )}>
    ...
    <div className="min-h-screen flex bg-muted/10">
      ...
      >
        <div className="h-16 flex items-center px-4 border-b border-sidebar-border/50">
          <Link href={`/${role}/dashboard`}>
            <a className={cn("flex items-center gap-2 font-bold text-xl text-sidebar-primary-foreground transition-opacity", !isSidebarOpen && "opacity-0 w-0 overflow-hidden")}>
              <div className="w-8 h-8 bg-primary text-primary-foreground flex items-center justify-center rounded-md shrink-0">S</div>
              <span className="whitespace-nowrap">SmartCON</span>
            </a>
          </Link>
          {!isSidebarOpen && (
             <div className="w-8 h-8 bg-primary text-primary-foreground flex items-center justify-center rounded-md shrink-0 mx-auto">S</div>
          )}
        </div>
      ...
      <div className={cn(
        ...
        "pb-16 md:pb-0" // Add padding bottom for mobile nav
      )}>
