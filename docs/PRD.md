...
**요구사항**: 한 사용자가 여러 역할을 동시에 가질 수 있습니다.

**예시 시나리오**:
- 현장 관리자이면서 동시에 팀장 역할을 수행하는 경우
- 팀장이면서 동시에 일반 노무자로도 출역하는 경우
- 본사 관리자가 특정 현장의 현장 관리자 역할도 수행하는 경우

**구현 방식**:
- 사용자-역할 매핑 테이블을 통해 다중 역할 관리
- 로그인 시 사용자가 보유한 모든 역할 목록 표시
- 사용자가 역할을 선택하여 해당 역할의 권한으로 시스템 이용
- 역할 전환 기능 제공 (상단 메뉴 또는 프로필 메뉴)
...
CREATE TABLE User_Roles (
    ...
    site_id BIGINT NULL FOREIGN KEY REFERENCES Sites(site_id), -- 역할별 현장 정보
    is_active BIT DEFAULT 1,
    assigned_at DATETIME DEFAULT GETDATE(),
    UNIQUE(user_id, role_code, site_id)
);
```
...
- 회원가입 시 역할(본사 관리자, 현장 관리자, 팀장, 노무자) 선택 (복수 선택 가능)
- 현장 관리자/팀장/노무자는 담당 현장 선택 필수
- 한 사용자가 여러 역할을 동시에 보유 가능

**역할 선택 프로세스**:
1. 소셜 로그인 완료 후 사용자 정보 조회
2. 사용자가 보유한 모든 역할 목록 표시
3. 사용자가 역할 선택 (단일 또는 다중 선택 가능)
4. 선택한 역할로 시스템 진입
5. 이후 역할 전환 기능 제공 (상단 메뉴 또는 프로필 메뉴)

**UI 요구사항**:
- 역할 선택 화면: 카드 형태로 각 역할 표시
  - 역할명, 설명, 권한 요약
  - 선택 가능한 역할은 체크박스 또는 라디오 버튼
- 역할 전환: 상단 헤더 또는 프로필 메뉴에서 드롭다운으로 역할 선택
...
-- 1-1. 사용자-역할 매핑 테이블 (User_Roles) - 다중 역할 지원
CREATE TABLE User_Roles (
    user_role_id BIGINT IDENTITY(1,1) PRIMARY KEY,
    user_id BIGINT NOT NULL FOREIGN KEY REFERENCES Users(user_id),
    role_code VARCHAR(10) NOT NULL, -- ROLE_ADMIN, ROLE_SITE, ROLE_TEAM, ROLE_WORKER
    site_id BIGINT NULL FOREIGN KEY REFERENCES Sites(site_id), -- 역할별 현장 정보 (현장 관리자, 팀장, 노무자용)
    team_name NVARCHAR(50) NULL, -- 팀명 (팀장, 노무자용)
    is_active BIT DEFAULT 1,
    assigned_at DATETIME DEFAULT GETDATE(),
    UNIQUE(user_id, role_code, site_id)
);
...
-- 인덱스
CREATE INDEX IX_User_Roles_UserId ON User_Roles(user_id);
CREATE INDEX IX_User_Roles_RoleCode ON User_Roles(role_code);
CREATE INDEX IX_User_Roles_SiteId ON User_Roles(site_id);
