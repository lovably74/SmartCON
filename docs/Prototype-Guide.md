...
router.beforeEach((to, from, next) => {
  const { currentUser, selectedRole, hasMultipleRoles } = useUser()
  
  if (to.meta.requiresAuth) {
    // 프로토타입에서는 인증 체크 생략
    // 역할 선택이 필요한 경우 체크
    if (to.meta.requiresRole && !selectedRole.value) {
      // 다중 역할을 가진 경우 역할 선택 화면으로 이동
      if (hasMultipleRoles.value) {
        next('/role-select')
      } else if (currentUser.value.roles.length === 1) {
        // 단일 역할인 경우 자동 선택
        selectRole(currentUser.value.roles[0])
        next()
      } else {
        next('/role-select')
      }
