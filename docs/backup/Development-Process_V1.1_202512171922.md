...
```
Phase 0: 프로토타입 개발 (3주) ⬆️ +1주
    ↓
Phase 1: 인증 및 사용자 관리 (6주) ⬆️ +2주
    ↓
Phase 2: 출역 체크 및 대시보드 (6주) ⬆️ +2주
    ↓
Phase 3: 작업일보 및 마이페이지 (4주) ⬆️ +1주
    ↓
Phase 4: 테스트 및 최적화 (3주) ⬆️ +1주
    ↓
Phase 5: 배포 및 안정화 (2주) ⬆️ +1주
```
...
**일정 조정 이유**:
- 초급 개발자 1명 단독 개발
- Cursor AI 학습 및 적응 시간 필요
- 코드 이해 및 검증 시간 필요
- 더 많은 테스트 및 디버깅 시간 필요

### 1.2. 개발 원칙

1. **Cursor 바이브코딩 우선**: 모든 개발은 Cursor AI와 함께 바이브코딩으로 진행
2. **이해하며 개발**: Cursor가 생성한 코드를 반드시 이해하고 학습
3. **프로토타입 우선**: 실제 개발 전 프로토타입으로 화면 검증
4. **테스트 주도**: 모든 기능 개발 후 테스트 코드 작성 필수
5. **반응형 우선**: PC/모바일 웹앱을 반응형으로 통합 개발
6. **코드 재사용**: 웹앱 코드를 모바일앱에서 최대한 재사용

**Cursor 바이브코딩 참고**: [바이브코딩 가이드](Pair-Programming-Guide.md) 문서의 "Cursor를 활용한 단독 바이브코딩" 섹션을 반드시 확인하세요.

### 1.3. 개발 환경

**개발자**: 초급 개발자 1명  
**협업 도구**: Cursor AI  
**개발 방식**: Cursor와의 바이브코딩 (AI Pair Programming)
...
**⚠️ 중요: 모든 개발은 바이브코딩으로 진행합니다.**

**작업 순서**:

1. **바이브코딩 세션 시작**
   - Driver: VS Code에서 Live Share 세션 시작
   - Navigator: 세션 링크로 참여
   - Discord 음성 채널 입장
...
3. **상태 관리 (Pinia)**
   ...
   - `user.store.ts`: 사용자 정보 관리 (다중 역할 포함)
   - `role.store.ts`: 현재 선택된 역할 관리

4. **API 서비스**
   - `api.service.ts`: HTTP 클라이언트 설정
   - `auth.service.ts`: 인증 API 호출
   - `user.service.ts`: 사용자 API 호출
   - `role.service.ts`: 역할 조회 및 선택 API
...
7. **역할 전환 기능**
   - 상단 헤더 또는 프로필 메뉴에 역할 전환 드롭다운
   - 역할 전환 시 해당 역할의 대시보드로 이동
...
| 1.1 | 2025-12-13 | 바이브코딩 프로세스 추가, 다중 역할 지원 반영 | Gemini |
| 1.2 | 2025-12-13 | Cursor 활용 단독 바이브코딩 반영, 개발 일정 조정 (24주) | Gemini |
